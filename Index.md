...
| `src/modules/Game/TutorialConfig.lua` | TutorialConfig | Defines the ordered objectives, final tutorial message, varied grease and Dust item pools, and forced restoration requirements used by the persistent, state-based onboarding flow. |
...
| `src/servercontrollers/GuidanceController.lua` | GuidanceController | Runs the visible tutorial through the Sponge purchase and sends its final message, then discreetly reconciles background onboarding milestones, selects varied guaranteed and mutation rewards, verifies upgrade ownership, applies one-time progression funding, tracks a suggested tutorial crate and the reward from the player's first broken crate, and sends contextual guidance. |
