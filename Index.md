...
| `src/controllers/BatController.lua` | BatController | Detects responsive crate and player bat targets, applies saved cooldown multipliers, cancels active swings and blocks Bat use while ragdolled, predicts fall-speed-scaled crate damage and lethal hits, renders size-normalized crate deformation, camera impact, audible hit feedback, and trailed local debris alongside Studio-authored critical-hit particles, shows replicated player swing animations, and sends prediction IDs for server reconciliation. |
...
| `src/modules/Game/CrateInfo.lua` | CrateInfo | Configures regular and pity-only crate tiers, shared size-normalized impact/debris/roll presentation, an unbounded high-tailed scale distribution averaging normal size, tier-adjusted size-based rarity luck, configurable missing-tool rarity penalties, economy-configured crate rarity odds, varied opening guaranteed drop pools used by state-based onboarding, nonzero all-rarity reward distributions, weighted item rolls, reward-time-balanced health, population limits, and the reset cycle. |
...
| `src/modules/Game/TutorialConfig.lua` | TutorialConfig | Defines the ordered objectives, final tutorial message, varied Paint, grease, and Dust item pools, and forced restoration requirements used by the persistent, state-based onboarding flow. |
...
| `src/servercontrollers/CrateController.lua` | CrateController | Uses the authored spawn region to replenish size-and-luck-varied crate fields in place on wall-clock-aligned resets, authoritatively applies damage from any bat and owns health, biases normal rarity rolls against tiers whose restoration tools are not owned, owns state-selected onboarding loot and per-reward restoration mixes, persists first-roll reveal timing, and manages free claiming and 30-second revealed reward lifetimes. |
...
| `src/servercontrollers/GuidanceController.lua` | GuidanceController | Runs the visible tutorial through the Paint purchase and sends its final message, then discreetly reconciles background onboarding milestones, guarantees the first post-purchase Paint and Sponge rewards require those tools, selects later varied mutation rewards, verifies upgrade ownership, applies one-time progression funding, tracks a suggested tutorial crate and the reward from the player's first broken crate, and sends contextual guidance. |
