...
| `src/modules/Game/CrateInfo.lua` | CrateInfo | Configures regular and pity-only crate tiers, a slightly large-skewed scale distribution centered on normal size, size-based rarity luck, four affordable new-player drops, normalized and validated rarity distributions, weighted item rolls, population limits, health/respawn pacing, and the reset cycle. |
...
| `src/servercontrollers/CrateController.lua` | CrateController | Uses the authored map regions to spawn size-and-luck-varied crate fields and return players in the crate zone to their museums during resets, authoritatively owns health and loot rolls, and manages reveals, purchases, despawns, and resets. |
