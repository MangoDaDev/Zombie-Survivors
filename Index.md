...
| `src/controllers/CrateController.lua` | CrateController | Starts predicted crate roulette immediately, waits for its minimum duration and authoritative result, then hands off through a dirty local reward model so network latency cannot leave a reveal gap. |
...
| `src/servercontrollers/CrateController.lua` | CrateController | Spawns tiered crate fields and pity crates, authoritatively owns health and loot rolls, pre-applies restoration damage before rewards become visible, and manages reveals, purchases, despawns, and resets. |
