...
| `src/servercontrollers/GuidanceController.lua` | GuidanceController | Runs the visible tutorial through the Sponge purchase, then discreetly reconciles background onboarding milestones, selects mutation rewards, verifies upgrade ownership, applies one-time progression funding, assigns the initial tutorial crate, and sends contextual guidance. |
...
| `src/UI/Menus/UpgradeTree.lua` | UpgradeTree | Provides a responsive STUD-style upgrade panel and opener with Sponge-only visible tutorial guidance followed by normal upgrade access, dynamic blue/grey purchase-state hexagons, a live purchasable-upgrade count, newly affordable alerts plus periodic ready reminders, BatInfo-driven bat icons, drag panning, zoom, reveals, and purchasing. |
