...
| `src/controllers/BatController.lua` | BatController | Detects responsive crate and player bat targets, applies saved cooldown multipliers, cancels active swings and blocks Bat use while ragdolled, predicts eligible crate damage and lethal hits while retaining weak-bat impact effects and showing temporary required-bat crate prompts, immediately renders local debris/break feedback, shows replicated player swing animations, and sends prediction IDs for server reconciliation. |
