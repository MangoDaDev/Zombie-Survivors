...
| Path | Name | Responsibility |
| --- | --- | --- |
| `src/servercontrollers/CarryController.lua` | CarryController | Owns stable per-copy carried and inventory item identity plus ownership metadata, assigns price-weighted restoration mixes to conveyor and legacy items, migrates legacy restoration state, builds restoration layers before welding so all damage follows carried items, and manages drops, movement, shared bases-area delivery, inventory, and tools. |
...
| `src/servercontrollers/CrateController.lua` | CrateController | Uses the authored spawn region to replenish size-and-luck-varied crate fields in place on wall-clock-aligned resets, authoritatively applies damage from any bat and owns health, normal and state-selected onboarding loot, per-reward price-weighted restoration mixes, persisted first-roll reveal timing, free claiming, and 30-second revealed reward lifetimes. |
