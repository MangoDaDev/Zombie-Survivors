...
| `src/controllers/InventoryController.lua` | InventoryController | Controls Satchel visibility, displays native Tool texture icons, requests carried-item drops, keeps the bat in the first slot, pages overflow tools through responsive mobile and PC hotbar arrows while preserving native equip controls, and sends validated inventory ordering to the server. |
