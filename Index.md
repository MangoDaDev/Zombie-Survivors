...
| `src/modules/Game/EconomyConfig.lua` | EconomyConfig | Centralizes economy tuning controls, progression-stage price bands, blended onboarding income, purchase/sale values, deliberately minor restoration bonuses, active and passive rewards, starting and recovery cash rules, readable rounding, and item/range validation. |
...
| `src/modules/Game/CrateInfo.lua` | CrateInfo | Configures regular and pity-only crate tiers, an unbounded high-tailed scale distribution averaging normal size, size-based rarity luck, four affordable new-player drops, normalized and validated rarity distributions, weighted item rolls, population limits, health, and the reset cycle. |
