...
| `src/controllers/CrateController.lua` | CrateController | Starts predicted crate roulette immediately, rolls until both its minimum duration and the authoritative reward result are ready, reconciles rejected predictions, and plays reveal VFX/audio and purchase guidance. |
...
| `src/servercontrollers/BatController.lua` | BatController | Supplies upgraded bats, validates predicted crate and PvP hits, echoes crate prediction IDs with authoritative results, enforces cooldown and anti-chain-hit protection, and applies server-owned forced drops, knockback, and temporary stuns. |
...
| `src/servercontrollers/CrateController.lua` | CrateController | Spawns tiered crate fields and pity crates, authoritatively owns health and loot rolls, reconciles predicted client reveals, and manages purchases, despawn countdowns, and area resets. |
