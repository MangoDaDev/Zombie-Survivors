...
| `src/controllers/BatController.lua` | BatController | Detects responsive crate and player bat targets, applies saved cooldown multipliers, predicts crate damage and lethal hits, immediately renders local debris/break feedback, shows replicated player swing animations, and sends prediction IDs for server reconciliation. |
...
| `src/controllers/FixingController.lua` | FixingController | Owns smoothly blended fixing cameras that fit each item's full rotating bounds with size-dependent margins, safe character-relative exit blending, mobile touch aiming offset above the finger, device-consistent viewport-relative cleaning/fixing columns with a small-item radius boost and moderate large-item bounds taper projected through the active item's full depth, immediate misaligned-part Hammer highlights, delayed fading hints on remaining restoration targets, grease-responsive rate-limited sponge audio, directional authored hairdryer airflow and debris motion, client-authoritative radius-assisted Hammer targeting with deterministic per-hit alignment and monotonic presentation progress, 85%-threshold assisted cleanup, responsive cleaning damage, contact-timed click/hold Hammer strikes, presentation feedback, Fix prompts, avatar hiding, and responsive surface tool/fake-arm viewmodels driven by world-near weighted and temporally stabilized surface normals. |
| `src/controllers/GuidanceController.lua` | GuidanceController | Resolves authoritative tutorial or contextual objectives, manages their local highlight, directional beam, and objective text, displays requested contextual alerts, and immediately clears completed interface guidance. |
...
| `src/controllers/MuseumVisitorController.lua` | MuseumVisitorController | Receives interest-scoped guest snapshots and commands through Networker, owns logical client guests, reports viewed-museum changes, and clears unsubscribed museum populations. |
...
| `src/modules/Game/EconomyConfig.lua` | EconomyConfig | Centralizes economy tuning controls, curved progression-stage price bands, blended onboarding income, purchase/sale values, restoration bonuses, active and passive rewards reaching endgame-scale guest payouts, starting and recovery cash rules, readable rounding, and item/range validation. |
...
| `src/modules/Game/CrateInfo.lua` | CrateInfo | Configures regular and pity-only crate tiers, an unbounded high-tailed scale distribution averaging normal size, size-based rarity luck, the opening guaranteed drops used by state-based onboarding, nonzero all-rarity reward distributions, weighted item rolls, reward-time-balanced health, population limits, and the reset cycle. |
...
| `src/modules/Game/TutorialConfig.lua` | TutorialConfig | Defines the ordered objectives and forced reward requirements used by the persistent, state-based onboarding flow. |
| `src/modules/Game/UpgradeConfig.lua` | UpgradeConfig | Defines the deterministic upgrade graph, economy-scaled costs, tier-aligned restoration chain, capacity, tool stats, eleven bat tiers, cooldowns, movement upgrades, cumulative tool costs, and graph/gating assertions. |
| `src/modules/Game/UpgradeLogic.lua` | UpgradeLogic | Resolves ownership, prerequisites, available and affordable upgrades, visibility, capacity, movement stats, automatic base Spray access, tool unlock sources and stats, bat tiers, and bat cooldown. |
...
| Path | Name | Responsibility |
| --- | --- | --- |
| `src/servercontrollers/CarryController.lua` | CarryController | Owns carried-item identity and ownership metadata, builds restoration layers before welding so all damage follows carried items, and manages drops, movement, shared bases-area delivery, inventory, and tools. |
| `src/servercontrollers/BatController.lua` | BatController | Supplies upgraded bats, validates and relays swing presentation, validates predicted crate and PvP hits against the same historical box volume used by the client, echoes authoritative results, and applies server-owned forced drops, knockback, re-hit protection, and temporary stuns. |
...
| `src/servercontrollers/GuidanceController.lua` | GuidanceController | Persists and reconciles onboarding milestones, selects forced mutation rewards, verifies upgrade ownership, applies one-time progression funding, assigns tutorial crates, and sends contextual guidance. |
| `src/servercontrollers/MuseumController.lua` | MuseumController | Builds museums, maintains cached occupied-display and museum-area indexes, repairs stale restoration state for already-displayed items, and handles placement, removal, server-validated confirmed selling, tutorial milestones, and level-aware visitor-facing exhibits. |
...
| `src/servercontrollers/UpgradeController.lua` | UpgradeController | Enforces focused early onboarding purchases before opening the price-guided Soft Brush stage, validates prerequisites and affordability, deducts cash, reconciles real tool ownership, and persists purchases. |
...
| `src/UI/Menus/UpgradeTree.lua` | UpgradeTree | Provides a responsive STUD-style upgrade panel and opener with focused Sponge onboarding followed by the price-guided Soft Brush stage, dynamic blue/grey purchase-state hexagons, a live purchasable-upgrade count, newly affordable alerts plus periodic ready reminders, BatInfo-driven bat icons, drag panning, zoom, reveals, and purchasing. |
