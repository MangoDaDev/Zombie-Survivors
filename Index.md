...
| `src/controllers/BatController.lua` | BatController | Detects responsive crate and player bat targets, applies saved cooldown multipliers, cancels active swings and blocks Bat use while ragdolled, predicts eligible crate damage and lethal hits while retaining weak-bat impact effects, immediately renders local debris/break feedback, shows replicated player swing animations, and sends prediction IDs for server reconciliation. |
...
| `src/controllers/GuidanceController.lua` | GuidanceController | Resolves authoritative tutorial or contextual objectives, targets only the player's assigned crate and its own reward during initial pickup guidance, manages local highlight, directional beam, and objective text, displays requested contextual alerts, and immediately clears completed interface guidance. |
...
| `src/modules/Game/CrateInfo.lua` | CrateInfo | Configures regular and pity-only crate tiers, the bat damage hit limit and required-bat lookup, an unbounded high-tailed scale distribution averaging normal size, size-based rarity luck, the opening guaranteed drops used by state-based onboarding, nonzero all-rarity reward distributions, weighted item rolls, reward-time-balanced health, population limits, and the reset cycle. |
...
| `src/servercontrollers/BatController.lua` | BatController | Supplies upgraded bats, validates and relays swing presentation, rejects and cancels Bat attacks during ragdoll, validates predicted crate and PvP hits against the same historical box volume used by the client, reports required bats for weak crate hits, echoes authoritative results, and applies server-owned forced drops, knockback, re-hit protection, and temporary stuns. |
