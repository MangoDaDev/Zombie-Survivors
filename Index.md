...
| `src/modules/Game/CrateInfo.lua` | CrateInfo | Configures regular and pity-only crate tiers, an unbounded high-tailed scale distribution averaging normal size, size-based rarity luck, four affordable new-player drops with a forced Dirt-and-Grease Toaster second, nonzero all-rarity reward distributions, weighted item rolls, reward-time-balanced health, population limits, and the reset cycle. |
| `src/modules/Game/DataTemplate.lua` | DataTemplate | Defines saved defaults for cash, guaranteed opening drops, persisted first-crate-roll state, inventory, museum displays, restoration state, tutorial progress, and upgrade ownership. |
...
| `src/servercontrollers/CrateController.lua` | CrateController | Uses the authored map regions to spawn size-and-luck-varied crate fields and return players in the crate zone to their museums during wall-clock-aligned crate-only resets, authoritatively owns health, normal and guaranteed onboarding loot rolls, persisted first-roll reveal timing, purchases, and naturally expiring reward items. |
