...
local function EnsureBat(Player)
	...
	for _, Container in { Player.Character, Player:FindFirstChildOfClass("Backpack") } do
		if Container then
			for _, Child in Container:GetChildren() do
				if Child:IsA("Tool") and type(Child:GetAttribute("BatId")) == "string" then return end
			end
		end
	end
...
local function IsTargetInRange(RootPart, TargetPosition, Info): boolean
	...
	return RootPart.CFrame.LookVector:Dot(Offset.Unit) >= Info.MinimumFacingDot
end
...
function BatController:Swing(Player, Targets)
	...
	for _, Target in Targets do
		...
		if Target:IsA("Model") and Target:HasTag("Crate") then
			if IsTargetInRange(RootPart, Target:GetPivot().Position, Info) then
		...
		if Target:IsA("Model") and Target ~= Character then
			...
			if Humanoid and TargetRoot and TargetRoot:IsA("BasePart") and TargetPlayer and Humanoid.Health > 0
				and IsTargetInRange(RootPart, TargetRoot.Position, Info)
			then
